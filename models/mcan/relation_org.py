...
class Relation_module(nn.Module):
    ...
    def attention_module_multi_head(self, feat, position_embedding, feat_mask, fc_dim, group, feat_dim, dim, nongt_dim=100):
        ...
        #print(feat.size()) feat [64,100,512]
        ...
        # WA
        aff = torch.matmul(q_data_batch, torch.transpose(k_data_batch, 2, 3)) #[64,32,100,100]
        #print(aff.size())
        aff_scale = (1.0 / math.sqrt(float(dim_group[1]))) * aff #[64,32,100,100]
        #print(aff_scale.size())
        aff_scale_1 = torch.transpose(aff_scale, 1, 2) #[64,149,32,149]
        aff_scale_2=torch.reshape(aff_scale_1, (n_batches, 100, fc_dim, nongt_dim))
        #print(aff_scale_2.size())

        # weighted_aff
        a=torch.log(torch.max(aff_weight, torch.full((n_batches, 100, fc_dim, nongt_dim), 1e-6).cuda())) 
        #print(a.size()) #torch.Size([64, 100, 32, 100])

        weighted_aff = a + aff_scale_1 #[64,100,32,100]
        #print(weighted_aff.size())
        weighted_aff = weighted_aff.masked_fill(feat_mask, -1e9)#[64,100,32,100]
        #print(weighted_aff.size())
        aff_softmax = F.softmax(weighted_aff, dim=3)#[64,100,32,100]
        #print(aff_softmax.size())
        aff_softmax_reshape = torch.reshape(aff_softmax, (n_batches, 100 * fc_dim, nongt_dim))#[64,100,32,100]
        #print(aff_softmax.size())
    ...
    def forward(self, feat, bbox, feat_mask):
        ...
        position_matrix = self.extract_position_matrix(bbox) #[64,100,100,64]
        position_embedding = self.extract_position_embedding(position_matrix) #[64,100,512]
        #print(position_embedding.size())

        attention_1 = self.attention_module_multi_head(feat, position_embedding, feat_mask, 32, 32, self.out_channel, dim=(self.out_channel, self.out_channel, self.out_channel)) #[64,100,512]
        #print(attention_1.size())
        new_feat = feat + attention_1 #[64,100,512]
        #print(new_feat.size())
        new_feat_relu = self.relu_2(new_feat)

        return new_feat_relu32

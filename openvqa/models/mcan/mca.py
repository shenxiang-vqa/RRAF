...
class SA(nn.Module):
    ...
    def forward(self, y, y_mask):
        y1, att = self.mhatt(y, y, y, y_mask)
...
class SGA(nn.Module):
    ...
    def forward(self, x, y, x_mask, y_mask):
        x1, att_sa = self.mhatt1(v=x, k=x, q=x, mask=x_mask)
        x = self.norm1(x + self.dropout1(
            x1
        ))
        
        x2, att_ga = self.mhatt2(v=y, k=y, q=x, mask=y_mask)
...
class MCA_ED(nn.Module):
    ...
    def forward(self, y, x, y_mask, x_mask):
        ...
        for dec in self.dec_list:
            x, x_sa, x_ga = dec(x, y, x_mask, y_mask)

        return y, x, y_sa, x_sa, x_ga
